...
from lib.rag.prompts import (
    format_qa_prompt,
    format_material_prompt,
...
class RAGPipeline:
    ...
    def __init__(self, gemini_api_key: str = None):
        ...
        self.embedder = GeminiEmbedder(gemini_api_key or env_key)
        ...
        api_key = gemini_api_key or env_key
        if api_key:
    ...
    def query(
        ...
        if not self.model:
            return {
                "answer": "Gemini API not configured. Please set GEMINI_API_KEY (or GOOGLE_API_KEY).",
                "sources": sources,
                "context": context,
            }
    ...
    def generate_material(
        ...
        **kwargs,
        ...
        if not self.model:
            return {
                "content": "Gemini API not configured. Please set GEMINI_API_KEY (or GOOGLE_API_KEY).",
                "sources": sources,
                "metadata": {},
            }
        ...
        return {
            ...
            "metadata": {
                "material_type": material_type,
                "generated_at": (
                    str(os.popen("date").read().strip()) if os.name != "nt" else ""
                ),
            },
        }
    ...
    def _get_platform_docs_context(self, question: str) -> str:
        ...
        try:
            docs_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "docs", "platform_docs.txt"
            )
            if os.path.exists(docs_path):
                ...
                for section in sections:
                    if any(
                        keyword in section.lower() for keyword in question_lower.split()
                    ):
